...
namespace etsi_its_cam_conversion {
  ...
  void convert_toRos(const INTEGER_t& _INTEGER_in, long& INTEGER_out) {
    ...
    if (status != 0) throw std::range_error("Failed to convert INTEGER_t to long");
  }

  template <typename T>
  void convert_toRos(const long& _INTEGER_in, T& INTEGER_out) {
    if (std::numeric_limits<T>::max() < _INTEGER_in) throw std::range_error("Failed to convert long to smaller integer type");
  ...
  void convert_toC(const long& _INTEGER_in, INTEGER_t& INTEGER_out) {
    ...
    if (status != 0) throw std::range_error("Failed to convert long to INTEGER_t");
  }
