...
#include <etsi_its_ivim_ts_conversion/convertRSCode.h>
#include <etsi_its_ivim_ts_msgs/msg/rs_code.hpp>
#include <etsi_its_ivim_ts_msgs/msg/road_sign_codes.hpp>
namespace ivim_ts_msgs = etsi_its_ivim_ts_msgs::msg;


namespace etsi_its_ivim_ts_conversion {

void toRos_RoadSignCodes(const ivim_ts_RoadSignCodes_t& in, ivim_ts_msgs::RoadSignCodes& out) {
  for (int i = 0; i < in.list.count; ++i) {
    ivim_ts_msgs::RSCode el;
    toRos_RSCode(*(in.list.array[i]), el);
    out.array.push_back(el);
  }
}

void toStruct_RoadSignCodes(const ivim_ts_msgs::RoadSignCodes& in, ivim_ts_RoadSignCodes_t& out) {
  memset(&out, 0, sizeof(ivim_ts_RoadSignCodes_t));
  for (int i = 0; i < in.array.size(); ++i) {
    ivim_ts_RSCode_t* el = (ivim_ts_RSCode_t*) calloc(1, sizeof(ivim_ts_RSCode_t));
    toStruct_RSCode(in.array[i], *el);
