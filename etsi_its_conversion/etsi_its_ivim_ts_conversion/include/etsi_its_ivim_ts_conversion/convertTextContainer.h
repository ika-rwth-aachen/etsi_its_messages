...
#include <etsi_its_ivim_ts_conversion/convertTcPart.h>
#include <etsi_its_ivim_ts_msgs/msg/tc_part.hpp>
#include <etsi_its_ivim_ts_msgs/msg/text_container.hpp>
namespace ivim_ts_msgs = etsi_its_ivim_ts_msgs::msg;


namespace etsi_its_ivim_ts_conversion {

void toRos_TextContainer(const ivim_ts_TextContainer_t& in, ivim_ts_msgs::TextContainer& out) {
  for (int i = 0; i < in.list.count; ++i) {
    ivim_ts_msgs::TcPart el;
    toRos_TcPart(*(in.list.array[i]), el);
    out.array.push_back(el);
  }
}

void toStruct_TextContainer(const ivim_ts_msgs::TextContainer& in, ivim_ts_TextContainer_t& out) {
  memset(&out, 0, sizeof(ivim_ts_TextContainer_t));
  for (int i = 0; i < in.array.size(); ++i) {
    ivim_ts_TcPart_t* el = (ivim_ts_TcPart_t*) calloc(1, sizeof(ivim_ts_TcPart_t));
    toStruct_TcPart(in.array[i], *el);
