...
void Converter::setup() {
  ...
  RCLCPP_INFO(this->get_logger(), "Converting UDP messages of type '%s' on '%s' to native ROS messages on '%s'", etsi_type_.c_str(), subscriber_udp_->get_topic_name(), publishers_cam_["denm"]->get_topic_name());
  RCLCPP_INFO(this->get_logger(), "Converting native ROS DENM messages on '%s' to UDP messages on '%s'", subscribers_cam_["denm"]->get_topic_name(), publisher_udp_->get_topic_name());
#endif
}
