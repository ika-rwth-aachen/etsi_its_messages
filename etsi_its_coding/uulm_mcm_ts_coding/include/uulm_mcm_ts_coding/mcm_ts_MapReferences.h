/*
 * Generated by asn1c-0.9.29 (http://lionet.info/asn1c)
 * From ASN.1 module "ETSI-ITS-CDD"
 * 	found in "/input/ETSI-ITS-CDD.asn"
 * 	`asn1c -fcompound-names -fprefix=mcm_ts_ -no-gen-BER -no-gen-XER -no-gen-JER -no-gen-OER -no-gen-example -gen-UPER`
 */

#ifndef	_mcm_ts_MapReferences_H_
#define	_mcm_ts_MapReferences_H_


#include <uulm_mcm_ts_coding/asn_application.h>

/* Including external dependencies */
#include <uulm_mcm_ts_coding/asn_SEQUENCE_OF.h>
#include <uulm_mcm_ts_coding/constr_SEQUENCE_OF.h>

#ifdef __cplusplus
extern "C" {
#endif

/* Forward declarations */
struct mcm_ts_MapReference;

/* mcm_ts_MapReferences */
typedef struct mcm_ts_MapReferences {
	A_SEQUENCE_OF(struct mcm_ts_MapReference) list;
	
	/* Context for parsing across buffer boundaries */
	asn_struct_ctx_t _asn_ctx;
} mcm_ts_MapReferences_t;

/* Implementation */
extern asn_TYPE_descriptor_t asn_DEF_mcm_ts_MapReferences;

#ifdef __cplusplus
}
#endif

/* Referred external types */
#include "uulm_mcm_ts_coding/mcm_ts_MapReference.h"

#endif	/* _mcm_ts_MapReferences_H_ */
#include <uulm_mcm_ts_coding/asn_internal.h>
